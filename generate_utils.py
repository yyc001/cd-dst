...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        irrelevant_prefix = ["decoder_", "cross_attn", "use_cache", 'teacher', 'student', 'st_', 'model_kwargs_student', 'use_cap_student', 'use_switch']
        encoder_kwargs = {
            argument: value
    ...
    def _get_generation_mode(
        ...
        if generation_config.teacher_student is not None:
            generation_mode = GenerationMode.TEACHER_STUDENT
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
        ...
        elif generation_mode == GenerationMode.TEACHER_STUDENT:
            # 11. prepare beam search scorer
            beam_scorer = BeamSearchScorer(
                batch_size=batch_size,
                num_beams=generation_config.num_beams,
            ...
            model_kwargs_student['attention_mask'] = model_kwargs['attention_mask']
            model_kwargs['model_kwargs_student'] = \
                model_kwargs['teacher_student']._prepare_encoder_decoder_kwargs_for_generation(
                    inputs_tensor,
                    model_kwargs_student,
                    model_input_name
                )
            return self.teacher_student_search(
                input_ids,
                beam_scorer,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
    ...
    def teacher_student_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs_student = model_kwargs['student_lm']. \
                prepare_inputs_for_generation(input_ids, **model_kwargs_student)
            ...
            next_token_scores_student = nn.functional.log_softmax(
                next_token_logits_student, dim=-1
            )  # (batch_size * num_beams, vocab_size)
            # next_token_scores_processed_student = logits_processor(input_ids, next_token_scores_student)
            # next_token_scores_student = next_token_scores_processed_student + beam_scores[:, None].expand_as(
            #     next_token_scores_processed_student
            # )
            ...
            vocab_size = next_token_scores.shape[-1]
            next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size)
            next_token_scores_student = next_token_scores_student.reshape(batch_size, num_beams * vocab_size)
            ...
            student_scores = torch.gather(next_token_scores_student, -1, next_tokens_keep)
            next_token_scores = torch.gather(next_token_scores, -1, next_tokens_keep)
            ...
            if model_kwargs_student["past_key_values"] is not None:
                model_kwargs_student["past_key_values"] = self._temporary_reorder_cache(
                    model_kwargs_student["past_key_values"], beam_idx
                )
